...
const navLinks = [
  ...
  { label: "Para quem e", href: "#personas" },
      ...
      >
        <div className="flex items-center justify-between h-14 md:h-16 pl-5 pr-2 md:pl-7 md:pr-2.5">
          ...
          <div className="hidden md:flex items-center gap-7 absolute left-1/2 -translate-x-1/2">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-sm text-foreground/75 hover:text-foreground transition-colors font-medium"
              >
                {link.label}
              </a>
            ))}
          ...
          <button
            ...
            className="md:hidden text-foreground pr-3"
        ...
        {mobileOpen && (
          ...
          >
            <div className="p-4 flex flex-col gap-3">
              ...
              <a
                href="/login"
                className="border border-border text-foreground font-bold py-3 rounded-full text-center uppercase tracking-wide"
              >
                Entrar
              </a>
              <a
                href="/cadastro"
                className="bg-ink text-ink-foreground font-bold py-3 rounded-full text-center mt-2 uppercase tracking-wide"
              >
                Criar conta
              </a>
            </div>
