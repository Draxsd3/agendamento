...
export default function AdminServices() {
  ...
  const openCreate = () => { setEditTarget(null); reset({ name: '', description: '', duration_minutes: 30, price: '' }); setShowModal(true); };
  const openEdit = (svc) => { setEditTarget(svc); reset(svc); setShowModal(true); };
  ...
  const columns = [
    { key: 'name', label: 'Nome' },
    { key: 'duration_minutes', label: 'Duração', render: (row) => `${row.duration_minutes} min` },
    { key: 'price', label: 'Preço', render: (row) => Number(row.price).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) },
    {
